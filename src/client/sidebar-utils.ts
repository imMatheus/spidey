/**
 * Tiny utilities the core bundle needs from "the sidebar". Lives in its own
 * file so importing it doesn't drag in `diff-sidebar.ts` (which transitively
 * pulls in `@pierre/diffs` and shiki — ~1.3 MB the core would otherwise have
 * to ship).
 *
 * The heavy `DiffSidebar` class is in [diff-sidebar.ts](./diff-sidebar.ts) and
 * is loaded on-demand via [lazy-diff-sidebar.ts](./lazy-diff-sidebar.ts).
 */
...
export function writePersistedSidebar(state: SidebarPersistedState | null) {
  try {
